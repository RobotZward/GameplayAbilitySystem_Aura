	...
	UAbilitySystemComponent* ASC, ULoadScreenSaveGame* SaveData)
{
	...
	const FGameplayEffectSpecHandle SpecHandle = ASC->MakeOutgoingSpec(ClassInfo->PrimaryAttributes_SetByCaller, 1.f, ContextHandle);
	...
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(SpecHandle, GameplayTags.Attributes_Primary_Resilience, SaveData->Resilience);
	...
	const FGameplayEffectSpecHandle SecondaryAttributesSpecHandle = ASC->MakeOutgoingSpec(
		ClassInfo->SecondaryAttributes_Infinite,
		1.f,
		SecondaryAttributesContextHandle);
	ASC->ApplyGameplayEffectSpecToSelf(*SecondaryAttributesSpecHandle.Data.Get());
	...
	const FGameplayEffectSpecHandle VitalAttributesSpecHandle = ASC->MakeOutgoingSpec(
		ClassInfo->VitalAttributes_Player,
		1.f,
		VitalAttributesContextHandle);
	ASC->ApplyGameplayEffectSpecToSelf(*VitalAttributesSpecHandle.Data.Get());
}
	...
	const float RadialDamageInnerRadius)
{
	if (FAuraGameplayEffectContext* AuraEffectContext = static_cast<FAuraGameplayEffectContext*>(
		EffectContextHandle.Get()))
	{
		AuraEffectContext->SetRadialDamageInnerRadial(RadialDamageInnerRadius);
	}
}
	...
	const float RadialDamageOuterRadius)
{
	if (FAuraGameplayEffectContext* AuraEffectContext = static_cast<FAuraGameplayEffectContext*>(
		EffectContextHandle.Get()))
	{
		AuraEffectContext->SetRadialDamageOuterRadial(RadialDamageOuterRadius);
	}
}
...
void UAuraAbilitySystemLibrary::SetKnockbackDirection(FDamageEffectParams& DamageEffectParams,
	FVector KnockbackDirection, float OverrideMagnitude)
{
	KnockbackDirection.Normalize();
	if (OverrideMagnitude > 0.f)
	{
		DamageEffectParams.KnockbackForceMagnitude = OverrideMagnitude;
	}
	DamageEffectParams.KnockbackForce = KnockbackDirection * DamageEffectParams.KnockbackForceMagnitude;
}

void UAuraAbilitySystemLibrary::SetDeathImpulseDirection(FDamageEffectParams& DamageEffectParams,
	FVector DeathImpulseDirection, float OverrideMagnitude)
{
	DeathImpulseDirection.Normalize();
	if (OverrideMagnitude > 0.f)
	{
		DamageEffectParams.DeathImpulseMagnitude = OverrideMagnitude;
	}
	DamageEffectParams.DeathImpulse = DeathImpulseDirection * DamageEffectParams.DeathImpulseMagnitude;
}

void UAuraAbilitySystemLibrary::SetTargetEffectParamASC(FDamageEffectParams& DamageEffectParams,

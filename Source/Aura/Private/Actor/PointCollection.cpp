	...
	float YawOverride)
{
	...
	for (USceneComponent* Pt : ImmutablePts)
	{
		...
		// 通过查询该Actor周围存活的Actor作为之后射线检测的IgnoreActor
		...
		const FVector AdjustedLocation = FVector(Pt->GetComponentLocation().X, Pt->GetComponentLocation().Y, HitResult.Location.Z);
